...
class _CobranzaScreenState extends State<CobranzaScreen> {
  ...
  loadDataFromApi() async{
    ...
    await provider.loadImputablesCobranza();
    await provider.getTiposPagos(5);
    await provider.getBancos();
    await provider.getNumber(3);
  ...
  loadComprobanteNumber() async{
    var numberResult= await dio.get('/api/modules/get-cobranza-number',
        queryParameters: {
          'clientId': me['clientId'],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          'Cobranza', style: TextStyle(color: AppColor.white),),
                  ...
                  children: [
                    ...
                    Positioned(
                        ...
                        child: CustomDropdownButton2(
                            ...
                            onChanged: (a) async {
                              if(cliente !=null){
                                originalData= await provider.getPendientesCobranza(cliente?.id ?? 0, cuenta?.id?? 0);
                      ...
                      top: 150, child: SizedBox(
                        ...
                        child: TextField(
                          readOnly: true,
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Recibo N°'), style: const TextStyle(
                            color: Colors.blue
                        )),
                      ),
                    ),
                    ...
                    Positioned(
                       ...
                       try{
                         ...
                         var result= await provider.generarCobranza({
                           'fecha': fecha.toIso8601String(),
                           'periodo': Globals.periodo,
