...
class _EgresoScreenState extends State<EgresoScreen> {
  ...
  var docController = TextEditingController(text: '000-001-');
                              ...
                              SizedBox(
                                ...
                                child: DropdownSearch<_Comprobante>(
                                  asyncItems: (f)=>dio.get('/api/modules/get-documentos', queryParameters: {
                                    'clientId': me['clientId'],
                                  }).then((value) => (value.data as List).map((e) => _Comprobante(e['name'],e['id'],e['auto'],e['valor'])).toList()),
                                  itemAsString: (u)=>u.name,
                                  dropdownDecoratorProps: const DropDownDecoratorProps(
                            ...
                            children: [
                              ...
                              ElevatedButton.icon(onPressed: asientoGenerado ? null: () async {
                                ...
                                var asiento= {
                                  ...
                                  'timbrado': 'N/A',
                                  'gravada10': (incluida10 / 1.1).roundToDouble(),
                                  'iva10': (incluida10 / 11).roundToDouble(),
                                  'gravada5': (incluida5 / 1.05).roundToDouble(),
                                  'iva5': (incluida5 / 21).roundToDouble(),
                                  'exenta': exentas,
                                  ...
                                  'centroCostoId': centro ? centroCosto?.id : null
                                };

                                var data= await provider.generarEgreso(asiento);
                                asientoId=data.$3;
                                listIngreso=data.$2;
...
class _Comprobante{
  ...
  _Comprobante(this.name, this.id, this.auto, this.value);
